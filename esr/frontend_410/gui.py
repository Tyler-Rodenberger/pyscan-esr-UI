...
class ExperimentUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.queue_manager.toggle_run_button.clicked.connect(self.run_queue)
    ...
    def initialize_from_queue(self):
        #Button logic
        self.set_parameters_and_initialize_btn.setEnabled(False)
        self.settings_changed = False

        # Update the Initialize indicator
        print("Reading and setting parameters from queue...\n")
        self.indicator_initialize.setStyleSheet(
            "background-color: red; border: 1px solid black; border-radius: 5px;"
        )
        if not self.queue_manager.currently_running:
            print("Trying to initialize from queue when no experiment currently running in queue manager")
            return
        self.current_experiment = self.queue_manager.currently_running.experiment
        tree = self.current_experiment.parameters
        root = tree.invisibleRootItem()
        new_params = self.current_experiment.parameters.copy()
        for i in range(root.childCount()):
            group_item = root.child(i)
    ...
    def stop_queue_running(self):
        self.worker.stop_sweep()

    def run_queue(self):
        if self.toggle_start_stop_sweep_frontend():
            self.initialize_from_queue()
            self.run_queue_experiment()
            if self.queue_manager.queue_running and self.queue_manager.active_queue.count() >= 1:
                self.run_queue()
            self.stop_queue_running()
...
class QueueManager(QWidget):
    ...
    def next_queue_item(self):
        ...
        if self.active_queue_list.count != 0:
            ...
            self.currently_running = next_experiment
            self.set_current_running(self.currently_running.display_name)
    ...
    def toggle_expand(self):
        ...
        self.collapsed_button.setText(f"Currently Running: {self.currently_running.display_name} {arrow}")
...
class QueuedExperiment(QListWidgetItem):
    ...
    def show_info_popup(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.experiment.parameters = self.parameters_dict.copy()
