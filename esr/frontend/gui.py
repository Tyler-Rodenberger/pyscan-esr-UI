...
class ExperimentUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.queue_manager.toggle_run_button.clicked.connect(self.start_stop_queue)
    ...
    def start_stop_queue(self):
        print("Starting or stopping the queue...")
        if self.queue_manager.queue_running:
            self.stop_queue(True)
        elif self.queue_manager.active_queue_list:
            self.queue_manager.toggle_run_button_text()
            self.read_processed_btn.setEnabled(False)
            self.read_unprocessed_btn.setEnabled(False)
            self.sweep_start_stop_btn.setEnabled(False)
            self.set_parameters_and_initialize_btn.setEnabled(False)
    ...
    def stop_queue(self, user_interrupt = False):
        ...
        if not user_interrupt and self.queue_manager.active_queue_list:
            ...
            self.queue_manager.queue_running = False
            self.queue_manager.toggle_run_button_text()
            self.set_parameters_and_initialize_btn.setEnabled(True)
            self.read_processed_btn.setEnabled(True)
            self.read_unprocessed_btn.setEnabled(True)
    ...
    def run_queue(self):
        ...
        #     experiment = self.queue_manager.get_next_experiment()
        #     print("Setting active experiment...", experiment.experiment.parameters)
        #     self.current_experiment = experiment.experiment
        #     print("Experiment updated")
        #     self.initialize_from_queue(experiment)
        #     print("Experiment initialized...")
        #     self.toggle_start_stop_sweep_frontend()
        # self.stop_queue()
        
    
    def initialize_from_queue(self, queued_experiment):
        """
        Initializes the experiment using parameters from a QueuedExperiment object
        instead of the settings panel.
        """
        self.settings_changed = False

        # Update the Initialize indicator
        print(f"Initializing from queue: {queued_experiment.parameters_dict['display_name']}")

        # Extract parameters from the queued experiment
        experiment = queued_experiment.experiment
        parameters = queued_experiment.parameters_dict["parameters"]

        # Set and initialize the experiment
        experiment.set_parameters(parameters)
        # print("Initializing pyscan experiment")
        # experiment.init_pyscan_experiment()
        # print("Initialized pyscan experiment successfully")

        # Hook experiment to UI
        self.current_experiment = experiment

        self.sweep_already_ran = False

        print("Initialized queued experiment with parameters:")
        for k, v in parameters.items():
            print(f"   {k}: {v}")
        print("Ready to run sweep.\n")
    ...
    def toggle_start_stop_sweep_frontend(self):
        if self.sweep_start_stop_btn.text() == "Start Sweep":
            ...
            print('Experiment being run in this sweep has parameters:', self.current_experiment.parameters)
...
class QueueManager(QWidget):
    ...
    def get_next_experiment(self):
        "Pops a QueuedExperiment object off the active queue if one exists and returns it"
        if self.active_queue_list:
            experiment = self.active_queue_list.takeItem(0)
...
class QueuedExperiment(QListWidgetItem):
    def __init__(self, start_stop_sweep_function, experiment: ExperimentType, queue_manager, last_used_directory=None, parameters_dict=None):
        ...
        self.display_name = self.generate_default_display_name()
        ...
        if parameters_dict:
            ...
            # Queued experiments are always going to be sweeps for us
            # May delete read_processed and read_unprocessed
    ...
    def show_info_popup(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.parameters_dict["sweep"] = True
